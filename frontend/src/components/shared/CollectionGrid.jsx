...
export default function CollectionGrid({ category, title, subtitle }) {
    ...
    const loadPage = useCallback(async (pageNum, replace = false) => {
        ...
        try {
            ...
            setHasMore(fetched.length === ROW_SIZE && pageNum < (result.pagination?.pages || 1));
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-white" style={{ overflowX: 'hidden', paddingTop: 'calc(var(--navbar-height) + 32px)' }}>
            {/* Header */}
            <header style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', marginBottom: '80px', textAlign: 'center' }}>
                <span style={{ color: '#C3AB7E', fontSize: '10px', fontWeight: '800', letterSpacing: '0.3em', textTransform: 'uppercase', display: 'block' }}>
                    COLLECTION
                </span>
                <h1 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontFamily: 'serif', marginTop: '12px' }}>{title}</h1>
                {subtitle && (
                    <p style={{ color: '#9ca3af', fontSize: '14px', maxWidth: '600px', margin: '16px auto 0' }}>
                        {subtitle}
                    </p>
                )}
            </header>

            <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px 160px' }}>
                {/* Initial skeleton */}
                {loadingInitial ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '100px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                        <Search size={48} style={{ color: '#d1ccc4' }} />
                        <h3 style={{ fontSize: '20px', fontFamily: 'serif', margin: 0 }}>Aucun produit trouvé</h3>
