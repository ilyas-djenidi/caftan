                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                        <div className="lg:col-span-8">
                            <div className="space-y-8">
                                {items.map((item) => (
                                    <div key={item.key} style={{ display: 'flex', gap: '32px', paddingBottom: '32px', borderBottom: '1px solid #f0ede8' }}>
                                        <div style={{ width: '160px', height: '200px', borderRadius: '24px', overflow: 'hidden', backgroundColor: '#f9f9f9' }}>
                                            <img src="/placeholder.jpg" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        </div>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                <div>
                                                    <h3 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '4px' }}>{item.product.name_fr}</h3>
                                                    <p style={{ color: '#9ca3af', fontSize: '14px' }}>
                                                        {item.size && `Taille: ${item.size}`} {item.color && ` • Couleur: ${item.color}`}
                                                    </p>
                                                </div>
                                                <span style={{ fontSize: '20px', fontWeight: '800' }}>{(item.product.price * item.quantity).toLocaleString()} DA</span>
                                            </div>
