...
export default function ProductDetail() {
        ...
        const loadProduct = async () => {
            try {
                ...
                if (productData.attributes) {
                    const sizes = productData.attributes.filter(a =>
                    ...
                    const colors = productData.attributes.filter(a =>
                    ...
                    const modelsAttr = productData.attributes.filter(a => a.type && a.type.toLowerCase() === 'model');
                ...
                const relatedRes = await getProducts({ limit: 4, category: productData.category });
                ...
                if (filteredRelated.length < 4) {
                    ...
                    const moreFiltered = (moreRes.data || []).filter(p => p.id !== productData.id && !filteredRelated.some(r => r.id === p.id));
    ...
    // DEBUG â€” check what attributes actually come back from Supabase
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-x-16" style={{ alignItems: 'start', gap: '24px 0' }}>
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Prev / Next arrows â€” only when multiple images */}
                            {images.length > 1 && (
                                <>
                                    <button
                                ...
                                {images.length > 1 && (
                                    <div style={{ display: 'flex', gap: '8px', marginTop: '24px', flexWrap: 'wrap' }}>
                                        {images.map((img, i) => (
                                            ...
                                            >
                                                <img loading="lazy" src={getImageUrl(img.image_url)} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} alt="" />
                                            </div>
                                        ))}
                                    </div>
                                )}
