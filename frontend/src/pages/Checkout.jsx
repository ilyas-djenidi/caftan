...
export default function Checkout() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (step === 1) {
            ...
            if (!formData.fullName.trim()) { alert('Veuillez entrer votre nom complet.'); return; }
            if (!formData.phone.trim()) { alert('Veuillez entrer votre numéro de téléphone.'); return; }
            if (!formData.wilaya) { alert('Veuillez sélectionner votre wilaya.'); return; }
            if (!formData.city) { alert('Veuillez sélectionner votre commune.'); return; }
            if (formData.deliveryType === 'home' && !formData.address.trim()) { alert('Veuillez entrer votre adresse complète.'); return; }
            setStep(2);
            window.scrollTo(0, 0);
            return;
        }
        ...
        try {
            const orderItems = items.map(item => {
                const combinedSize = item.model
                    ? (item.size ? `${item.size} (Modèle ${item.model})` : `Modèle ${item.model}`)
                    : (item.size || null);
            ...
            setOrderSuccess({ order_number: result.order_number });
            clearCart();
            setStep(3);
        } catch (error) {
            console.error('Checkout error:', error);
            const msg = error?.response?.data?.error || error?.response?.data?.message || 'Une erreur est survenue lors de la commande. Veuillez réessayer.';
            alert(msg);
        } finally {
            setLoading(false);
        }
