...
import { getAdminProducts, getAdminProduct, createProduct, updateProduct, deleteProduct } from '../../api/products.api';
...
export default function Products() {
    ...
    const handleOpenModal = async (product = null) => {
        ...
        if (product) {
            ...
            try {
                const fullProduct = await getAdminProduct(product.id);
                    ...
                    {products.map(product => (
                        <div key={product.id} style={{ backgroundColor: 'white', borderRadius: '24px', border: '1px solid #f0ede8', overflow: 'hidden' }} className="group">
                            <div style={{ aspectRatio: '1/1', backgroundColor: '#fafafa', position: 'relative' }}>
                                <img loading="lazy" src={product.images?.[0]?.image_url || '/placeholder.jpg'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
