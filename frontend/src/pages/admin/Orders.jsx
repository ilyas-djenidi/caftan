    ...
    useEffect(() => {
        const load = async () => {
            ...
            await fetchOrders({
                ...
                guepex_status: guepexFilter === 'ALL' ? null : guepexFilter,
    ...
    const handleExpandRow = async (orderId) => {
        if (expandedOrderId === orderId) {
            ...
            try {
                ...
                console.log('[DEBUG] expandedOrderDetails:', result);
                console.log('[DEBUG] order_number:', result?.order_number);
                setExpandedOrderDetails(result);
                
                // Calculate local fallback fee + fetch live from Guepex
                if (result?.wilaya) {
                    const fee = getDeliveryFee(result.wilaya, result.notes?.includes('Bureau') ? 'bureau' : 'home');
                    setDetectedRate({ fee });
                    getWilayas().then(wData => {
                        ...
                        const match = list.find(w => norm(w.name) === norm(result.wilaya));
                        if (match) {
                            const fromId = Number(import.meta.env.VITE_STORE_WILAYA_ID) || 28;
                            const isBureau = result.notes?.includes('Bureau');
