...
export default function Home() {
    ...
    const handleContactSubmit = async (e) => {
        ...
        try {
            await sendMessage({
                ...
                subject: 'General Inquiry', message: contactData.message,
            });
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            {/* CATEGORIES GRID */}
            <section style={{ padding: 'clamp(60px, 10vw, 120px) 0', backgroundColor: '#ffffff', overflow: 'hidden' }}>
                        ...
                        {categories.map((cat, i) => (
                            <Link key={i} to={cat.to} style={{
                                ...
                                <div style={{
                                    ...
                                    <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: '9px', fontWeight: '800', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{cat.count}</span>
                                    <h3 style={{ color: 'white', fontSize: '22px', fontFamily: 'serif', margin: 0, lineHeight: 1.1 }}>{cat.name}</h3>
                                    <div style={{
                                        width: '32px', height: '2px', backgroundColor: '#C3AB7E',
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                        {loading ? (
                            ...
                            caftansProducts.slice(0, 3).map((product) => (
                                <ProductCard
                                    key={product.id}
                                    product={product}
            ...
            <section style={{ padding: '0 0 80px', backgroundColor: '#ffffff' }}>
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
                    <div style={{ textAlign: 'center', marginBottom: '80px' }}>
                        <h2 style={{ fontSize: '48px', fontFamily: 'serif', margin: 0 }}>{t('nav.jellabas')}</h2>
                        <p style={{ color: '#9ca3af', fontSize: '14px', marginTop: '16px', maxWidth: '500px', margin: '16px auto 0' }}>
                            {t('home.categories.subtitle')}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                        {loading ? (
                            ...
                            jellabasProducts.slice(0, 3).map((product) => (
                                <ProductCard
                                    key={product.id}
                                    product={product}
                                    onClick={() => navigate(`/product/${product.id}`)}
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                        {loading ? (
                            ...
                            accessoiresProducts.slice(0, 3).map((product) => (
                                <ProductCard
                                    key={product.id}
                                    product={product}
                                    onClick={() => navigate(`/product/${product.id}`)}
