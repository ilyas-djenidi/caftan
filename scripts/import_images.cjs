/**
 * import_images.cjs
 * Imports product_attributes from CSV into Supabase.
 * Run from project root: node scripts/import_images.cjs
 */
...
function parseCSVSimple(text, stripCols = []) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        headers.forEach((h, idx) => {
            const col = h.trim();
            if (stripCols.includes(col)) return;
...
async function importTable(csvFile, tableName, label, stripCols = []) {
    ...
    const rows     = parseCSVSimple(fs.readFileSync(filePath, 'utf-8'), stripCols);
...
async function run() {
    console.log('🚀 Starting full data import into Supabase...');
    ...
    await importTable(
        ...
        'product_attributes',
        'Product Attributes',
        ['is_available', 'label', 'display_order', 'stock', 'hex_color', 'image_url'],
    );
